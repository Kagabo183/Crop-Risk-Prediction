...
async def trigger_data_fetch(background_tasks: BackgroundTasks):
    ...
    This runs in the background and updates data up to today's date.
    """
    from scripts.auto_fetch_data import main as fetch_data
    
    background_tasks.add_task(fetch_data)
    
    return {
        "status": "success",
        "message": "Data fetch started in background. Check logs for progress."
    }
